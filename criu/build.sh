...
if [ ! -f source/.criu_patched ]; then
    patch -d source/criu-* -p1 < patches/criu-success-v2.9.patch
...
if [ ! -f source/.criu ]; then
    ...
    make clean
    export ARCH=arm64
    export WERROR=1 CRIU_MAKE_OPTS="ARCH=$XARCH CROSS_COMPILE=/usr/bin/aarch64-linux-gnu- USERCFLAGS=\"-I$XPREFIX/include -I$XPREFIX/include/libnl3 -I/usr/aarch64-linux-gnu/include -L/usr/aarch64-linux-gnu/lib -L/$XPREFIX/lib64 -L/$XPREFIX/lib -lprotobuf-c -lpthread -lrt\""
    make WERROR=1 CROSS_COMPILE=/usr/bin/aarch64-linux-gnu- USERCFLAGS="-I$XPREFIX/include -I$XPREFIX/include/libnl3 -I/usr/aarch64-linux-gnu/include" LDFLAGS="-L/usr/aarch64-linux-gnu/lib -L/$XPREFIX/lib64 -L/$XPREFIX/lib -lprotobuf-c -lpthread -lrt" V=1
fi
